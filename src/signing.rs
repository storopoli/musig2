    ...
    fn test_partial_sign_and_verify() {
        ...
        for (test_index, test_case) in vectors.valid_test_cases.into_iter().enumerate() {
            let pubkeys: Vec<Point> = test_case
                ...
                .into_iter()
                .map(|i| {
                    Point::try_from(&vectors.pubkeys[i]).unwrap_or_else(|_| panic!("invalid pubkey used in valid test: {}",
            ...
            let aggregated_nonce = AggNonce::from_bytes(aggnonce_bytes).unwrap_or_else(|_| panic!("invalid aggregated nonce used in valid test case: {}",
            ...
            let public_nonces: Vec<PubNonce> = test_case
                ...
                .into_iter()
                .map(|i| {
                    PubNonce::from_bytes(&vectors.public_nonces[i]).unwrap_or_else(|_| panic!("invalid pubnonce in valid test: {}",
