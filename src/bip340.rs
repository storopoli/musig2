    ...
    fn test_bip340_signatures() {
        ...
        for result in csv_reader.deserialize() {
            ...
            if let Some(seckey) = record.seckey {
                let aux_rand = <[u8; 32]>::try_from(record.aux_rand.as_slice()).unwrap_or_else(|_| panic!("invalid aux_rand: {}",
            ...
            match record.verification_result.as_str() {
                "TRUE" => {
                    verify_result.unwrap_or_else(|_| panic!("verification should pass for signature {} - {}",
