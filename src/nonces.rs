...
mod tests {
    ...
    fn test_nonce_generation() {
        ...
        for test_case in vectors.test_cases {
            let aggregated_pubkey = Point::lift_x(&test_case.aggregated_pubkey).unwrap_or_else(|_| panic!("invalid aggregated xonly pubkey in test vector: {}",
    ...
    fn test_nonce_aggregation() {
        ...
        for test_case in vectors.valid_test_cases {
            let nonces: Vec<PubNonce> = test_case
                ...
                .into_iter()
                .map(|i| {
                    PubNonce::from_bytes(&vectors.public_nonces[i]).unwrap_or_else(|_| panic!("used invalid nonce in valid test case: {}",
        ...
        for test_case in vectors.error_test_cases {
            for (signer_index, i) in test_case.public_nonce_indexes.into_iter().enumerate() {
                ...
                if signer_index == test_case.error.signer {
                    ...
                    nonce_result.unwrap_or_else(|_| panic!("unexpected pub nonce parsing error for signer {}",
                        i));
                }
            }
        }
